...
ADMISSION_NAME              := shoot-networking-filter-admission
...
docker-images:
	...
	@docker build --build-arg EFFECTIVE_VERSION=$(EFFECTIVE_VERSION) -t $(IMAGE_PREFIX)/$(NAME)-admission:$(VERSION) -t $(IMAGE_PREFIX)/$(NAME)-admission:latest -f Dockerfile -m 6g --platform $(PLATFORM) --target $(EXTENSION_PREFIX)-$(ADMISSION_NAME) .
...
.PHONY: generate
generate: $(CONTROLLER_GEN) $(GEN_CRD_API_REFERENCE_DOCS) $(HELM) $(YQ) $(VGOPATH) $(EXTENSION_GEN) $(KUBECTL)
